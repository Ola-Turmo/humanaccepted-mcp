...
function crockfordRandom(length: number): string {
  ...
  let out = "";
  for (let i = 0; i < length; i++) out += ALPHA[bytes[i] % 32];
  return out;
}
...
interface Receipt {
  id: string;
  kind: "tool_call" | "sub_agent" | "model_invocation";
  version: 2;
}

async function postReceipt(path: string, body: any): Promise<Receipt> {
...
async function instrumentedToolCall(opts: {
  ...
  args: unknown;
  callFn: () => Promise<any>;
}): Promise<{ result: any; startReceiptId: string; endReceiptId: string }> {
  ...
  let result: any;
  let resultKind: "ok" | "error" | "timeout" | "denied" = "ok";
  try {
...
const server = new Server(
  { name: "humanaccepted-mcp", version: "0.1.0" },
  { capabilities: { tools: {} } }
);

// Per-session state.
let sessionRunId: string | null = null;
let sessionLastReceiptId: string | null = null;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (!sessionRunId) sessionRunId = newRunId();
